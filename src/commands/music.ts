/*
Code from this repo: 
    https://github.com/discordx-ts/plugins/tree/main/plugins/ytdl-player
License:
    https://github.com/discordx-ts/plugins/blob/main/plugins/ytdl-player/LICENSE
...
export class Music {
  ...
  async spotify(
    ...
    const videos = await Promise.all(
      // @ts-ignore
      result.map((track) =>
        YouTube.searchOne(`${track.title} by ${track.artist}`)
      )
    );
